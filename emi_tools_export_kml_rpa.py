...
class emiToolsExportKmlRpa(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterField(
                "export_field", tr("Export file name field"), "", "layer", optional=True
            )
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        for i, feature in enumerate(features):
            ...
            # Sanitize filename
            raw_value = (
                feature[export_field]
                if export_field and feature[export_field]
                else f"{layer.sourceName()}_{feature.id()}"
            )
            field_value = (
                str(raw_value).replace(".", "_").replace("/", "_").replace("\\", "_")
            )
            ...
            feedback.setProgress(
                (i + 1) / total_features * 100 if total_features else 0
            )
    ...
    def load_layers(self, files):
        ...
        QgsProject.instance().addMapLayers(
            [layer_item for layer_item in layers if layer_item.isValid()]
        )
